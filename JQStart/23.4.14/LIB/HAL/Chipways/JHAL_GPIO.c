...
void  JHAL_gpioModeSet(JHAL_IO_Port port,u8 pin,JHAL_IO_MODE mode)
{
    if((u8)mode<(u8)JHAL_IO_PP)
    {
        ...
        if(mode!= JHAL_IO_IN )
        {
            ...
            //手册247页
            SIM_PINSEL_IRQ((u8)(port/2));	//映射第4组引脚 PTGx + PTHx  IRQ_PS_GROUP4


            NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);
            NVIC_InitTypeDef NVIC_InitStruct;
            NVIC_InitStruct.NVIC_IRQChannel = EXTI0_IRQn+((port%2)*8)+pin;
            ...
            IRQ_InitStruct.IRQ_EdgeSelect = JHAL_IO_IN_EXTI_RET==mode? IRQ_Riseedge_Highlevel:IRQ_Falledge_Lowlevel;
            IRQ_InitStruct.IRQ_ModeSelect = IRQ_Detect_Edge;
            //define  EXTI0                 0                     < 外部中断通道0.
            IRQ_Init(IRQ,((port%2)*8)+ pin, &IRQ_InitStruct); //中断触发设置  下降沿
            IRQ_PinEnableCmd(IRQ, ((port%2)*8)+pin, ENABLE); //引脚使能
            IRQ_InterruptEn(IRQ, ((port%2)*8)+pin, ENABLE);//中断使能
            IRQ_PullDevDisableCmd(IRQ,((port%2)*8)+pin,ENABLE);
        }
		

    } else if((u8)mode<(u8)JHAL_IO_OD)
    {
