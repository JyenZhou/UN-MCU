...
void __JHAL_i2csfStop(JHAL_I2CSF  *config)
{
    ...
    {
        JHAL_gpioModeSet(config->sclPort,config->sclPin, JHAL_IOMODE_IO_OutputPushPull |JHAL_IOMODE_PullResistor_Up);
        JHAL_gpioModeSet(config->sdaPort,config->sdaPin, JHAL_IOMODE_IO_OutputPushPull |JHAL_IOMODE_PullResistor_Up);

    } else {
        JHAL_gpioModeSet(config->sclPort,config->sclPin, JHAL_IOMODE_IO_OutputDrain |JHAL_IOMODE_PullResistor_Up);
        JHAL_gpioModeSet(config->sdaPort,config->sdaPin, JHAL_IOMODE_IO_OutputDrain |JHAL_IOMODE_PullResistor_Up);
...
bool __JHAL_i2csfWaitAck(JHAL_I2CSF  *config)
{
    ...
    if(config->isIO_Mode_PP)
    {
        JHAL_gpioModeSet(config->sdaPort,config->sdaPin,JHAL_IOMODE_IO_Input|JHAL_IOMODE_PullResistor_Up);
    }
    JHAL_delayUs( config->delayMultiple);
    JHAL_gpioWitePin(config->sclPort,config->sclPin,true);
    JHAL_delayUs( config->delayMultiple);
...
bool JHAL_i2csfMemRead(JHAL_I2CSF  * config, u16 memAddress,bool memSizeIs16,u8 *pData, u8 dataSize)
{
    ...
    return  JHAL_i2csfReceice(config,memData,memSizeIs16?2:1,false,pData,dataSize);
